...
const SearchBar = ({ isMobile = false }) => {
  ...
  const searchElasticsearch = async (term) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:9200/restaurantes_v3/_search', {
        method: 'POST',
        headers: {
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400 flex justify-between">
                      ...
                      {/* Distancia entre la ubicación actual al restaurante PENDIENTE */}
                      <span>4.5 km</span>
                    </div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">{Array.isArray(restaurant.categoria) ? restaurant.categoria.join(", ") : restaurant.categoria}</div>
                  </div>
                </div>
